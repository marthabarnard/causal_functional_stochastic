...
true_out <- gen_sim_no_func(seed = 5, n=1000000, p=10, b1=input_grid$b1[i], b2=2, b3=input_grid$b3[i],
                   t1=150, t2=200, corr=input_grid$corr[i], q_sd = input_grid$q_sd[i],
...
write_rds(c(true_mu, true_mu_q, lb, ub), paste0('true_res_Q/', i, '.rds'))
...
res <- mclapply(seq(10000001, 10000001+999), gen_sim,
              ...
              lb = lb, ub = ub, mc.cores = 25)

est_res <- mclapply(seq(1, length(res)), function(m){
  print(paste0('df # ', m))
  final_df <- lapply(c(0.99, 0.999), function(k){
    ...
    lincde_fpca <- tryCatch(
      {
        ...
        print('lincde fpca')
        dr_lincde1_fpca  <- get_w3_t2(n.trees = 1000, depth = 2, centering = F, centeringMethod = "linearRegression",
                                   design_pts, delta_vec, mod_df1, mod_df2, 'coef1_2', hyper_grid, m*2000)
        dr_lincde2_fpca  <- get_w3_t2(n.trees = 1000, depth = 2,centering = F, centeringMethod = "linearRegression",
    ...
    true_fpca <- tryCatch(
      {
        ...
        print('true fpca')
        dr1_fpca  <- get_w1(q_sd, design_pts, delta_vec, mod_df1 , mod_df2 , 'coef1_2', hyper_grid, m*2000)
        dr2_fpca  <- get_w1(q_sd,design_pts, delta_vec, mod_df2 , mod_df1 , 'coef1_2', hyper_grid, m*4020)
        
        cbind(type = 'est_true', get_est(dr1_fpca, dr2_fpca, mod_df1$y, mod_df2$y, delta_vec))
      },
      error = function(e) {
        fake_df <- as.data.frame(matrix(rep(NA, 50), nrow = 5))
        colnames(fake_df) <- c('type', 'delta', 'out_mod', 'w_norm', 'dr_norm', 'dr_norm2', 'var_est', 'var_est2', 'var_contrast_est',  'var_contrast_est2')
        fake_df$type = 'est_true'
        return(fake_df)
      })
    ...
    final <- as.data.frame(rbind(lincde_fpca, true_fpca)) %>%
      mutate(seed = m,
             est_y = mean(c(mod_df1$y, mod_df2$y)),
             true_mu = true_mu,
             true_mu_q = rep(true_mu_q, 2),
             pve = k,
             lb = lb,
             ub = ub,
             n_col = ncol(mod_df),
...
write_rds(final_res, paste0('res_Q/', i, '.rds'))
